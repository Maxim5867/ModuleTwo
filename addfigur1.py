...
def addFigur():
    ...
    v = IntVar()
    v.set(1)
    Radiobutton(fig, text='Прямоугольник', variable=v, value=1).grid(row=6,columnspan=7)
    Radiobutton(fig, text='Овал', variable=v, value=0).grid(row=7,columnspan=3)
    ...
    Button(fig, text='Нарисовать', command=paint).grid(row=9,columnspan=7)
